  ...
  $(document).on("click", ".musicas_playlist", function() {
    ...
    if (id_playlist != '-') {
      ...
      findAllMusicas(id_playlist,
        ...
        function() {
          showResponse('Erro', 'Falha ao buscar músicas', 'error');
        }
      );
    }
  });   
      ...
      if ($colId.text() != '-')
        updateMusica(id_playlist, $colId.text(), musica,
          function() { 
            ...
            toggleAttr($tr);
            showResponse('Sucesso', 'Música atualizada com sucesso', 'success');
          },
          function() {
            showResponse('Erro', 'Falha ao atualizar música', 'error');
          }
        );
      else {
        insertMusica(id_playlist, musica, 
          function(id) { 
            ...
            toggleAttr($tr);
            showResponse('Sucesso', 'Música adicionada com sucesso', 'success');
          },
          function() {
            showResponse('Erro', 'Falha ao adicionar música', 'error');
          }
        );
      }
    }   
  ...
  $(document).on("click", ".delete_musica", function(){
    ...
    if (id_musica != '-') {
      ...
      removeMusica(id_playlist, id_musica, 
        function() { 
          $parent.remove(); 
          showResponse('Sucesso', 'Música removida com sucesso', 'success');
        },
        function() {
          showResponse('Erro', 'Falha ao remover música', 'error');
        }
      );
    } else {
      toggleAttr($(this));
